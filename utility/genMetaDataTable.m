...
xlsxPath = "D:\EEG\HarMNqEEG\TDBRAIN_participants_V2.xlsx";
infoData = readtable(xlsxPath,"Sheet","TDBRAIN-V2");


subjList = dir([data_path, '/*.mat']);              % List all .mat files
fields={'name','country','age','sex','ref','device','indication','formal_status','study',...
...
for j=nsubs:-1:1
    try
        ...
        rowindex = find(strcmp(infoData.participants_ID, data_struct.name(1:end-10)));
        ...
        data_table(j).indication = [infoData.indication{rowindex}];
        data_table(j).formal_status = [infoData.formal_status{rowindex}];
    catch ME
